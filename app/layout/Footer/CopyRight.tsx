        ...
        <div className="flex flex-wrap-reverse items-start gap-3 sm:gap-4 sm:ml-4">
            ...
            <Suspense>
                <Await resolve={footer}>
                    {resolve => resolve.menu.items.map((item: any) => <Link className="text-sm hover:underline underline-offset-4 opacity-60 hover:opacity-90" key={item.url} to={item.url}>{item.title}</Link>)}
                </Await>
            </Suspense>
        </div>
    )
